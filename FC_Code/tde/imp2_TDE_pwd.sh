...
alter database open;
administer key management set keystore open identified by "${WALLET_PWD}";
ADMINISTER KEY MANAGEMENT IMPORT ENCRYPTION KEYS WITH SECRET "${WALLET_SECRET}" FROM '${TDE_EXP_FILE_NAME_2}' IDENTIFIED BY "${WALLET_PWD}" with backup;
ADMINISTER KEY MANAGEMENT USE KEY '${WALLET_KEY_2}' IDENTIFIED BY "${WALLET_PWD}" WITH BACKUP;
EOF
